...
double euler(double k, int n, int pasosPorPeriodo, double periodoIntegrado, double theta, double omega){
        ...
        for(int i=0;i<n;i++){
               ...
               // cout << theta << endl;
                /*if(i%pasosPorPeriodo == 0) cout << "\tPeriodo " << i/pasosPorPeriodo << " - Amplitud:" << theta << endl;


                if(omega<0 && signoOmega == 1){
                        //Pasó de positivo a negativo
                        cout << "\tMaxima amplitud para T=" << k*i << " Esperado: " << periodoIntegrado*per << " ERel:" << (k*i-periodoIntegrado*per)/(k*i) << endl;
                        per++;
                }*/
